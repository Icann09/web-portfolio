...
export default function Services() {
  return (
    <section className="py-16 px-6 text-center min-h-[calc(100vh-4rem)] max-w-6xl mx-auto " id="services">
      ...
      <h2 className="text-3xl font-bold mb-4 dark:text-[#948979]">My services</h2>
      ...
      <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
        {services.map((service, index) => (
          <div
            key={index}
            className={`p-8 rounded-xl border bg-white shadow-sm hover:shadow-md hover:bg-gray-100 transition ${
              service.highlight
                ? "bg-pink-50 border-pink-100 shadow-lg"
                : "bg-white"
            }`}
          >
            <div className="flex items-center justify-center w-12 h-12 rounded-full bg-gray-950 mb-4 mx-auto">
              {service.icon}
            </div>
